...
ggplot(dados_aluguel, 
  ...
  geom_boxplot() + 
  labs(title = "Distribuição do preço de aluguel por quantidade de piscina",
       x = "Piscina",
...
```{r}
mod_linear <- lm(price ~ ., dados_treinamento)

pred_linear <- predict(mod_linear, dados_validacao)

df_pred <- tibble(y_obs = dados_validacao$price,
                  y_pred = pred_linear,
                  modelo = "linear")
```
...
```{r}
library(rpart)
mod_arvore <- rpart(price ~ ., dados_treinamento)
...
df_pred <- df_pred %>% 
  bind_rows(tibble(y_obs = dados_validacao$price,
                   y_pred = pred_arvore,
                   modelo = "árvore")) 
...
```
::: {.callout-tip title="Dica" collapse="false"}
Observe que os valores preditos pela árvore de decisão apresentam um padrão (linhas horizontais) por conta da forma como o modelo é contruído!
:::
